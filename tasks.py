...
def pypi(c):
    ...
    c.run(f'python -m twine upload dist/circuitbrew-{ver}-py3-none-any.whl')
...
def tests(c):
    test_dir = 'test'
    c.run('pytest')
    c.run('pytest --cov-config .coveragerc --cov=circuitbrew --cov-report=term --cov-report=html')
    c.run('coveralls')


@task
def docs(c):
    githubpages = "/Users/virantha/dev/githubdocs/circuitbrew"
    ...
    with c.cd("docs"):
        print("Running sphinx in docs/ and building to ~/dev/githubpages/circuitbrew")
        c.run("make clean")
